...
const CreateParcelForm = ({ onSubmit, receiverOptions }: CreateParcelFormProps) => {
  const { toast } = useToast();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [formData, setFormData] = useState({
    receiverId: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Validate required fields
      const requiredFields = ['receiverId', 'type', 'weight', 'fee', 'deliveryDate'];
      ...
      // Validate weight and fee are positive numbers
      const weight = parseFloat(formData.weight);
      ...
      toast({
        title: "Parcel Created",
        description: "Your parcel has been created successfully.",
      });
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(value) => handleSelectChange("receiverId", value)}
          >
            <SelectTrigger>
