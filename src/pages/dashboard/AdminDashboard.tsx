...
const AdminDashboard = () => {
  ...
  const { data: parcelsResponse, isLoading: parcelsLoading } = useGetAllParcelsQuery();
  const { data: usersResponse, isLoading: usersLoading } = useGetAllUsersQuery();
  
  const allParcels = parcelsResponse?.data?.parcels || [];
  ...
  const userStats = {
    ...
    active: allUsers.filter(u => u.isActive === 'ACTIVE').length,
    blocked: allUsers.filter(u => u.isActive === 'BLOCKED').length,
  };
